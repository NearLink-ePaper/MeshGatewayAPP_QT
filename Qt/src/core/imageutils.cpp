                                               ...
                                               int jpegQuality)
{
    ...
    // 4bpp RAW 模式：≤240×360 可放入设备缓冲区 (43200B < 48KB)
    // FS 抖动量化 → nibble 打包，固件 EPD_display_4bpp 内部做 90° CW 旋转
    ...
    // JPEG 模式：大分辨率 (480×800) 原始 4bpp 超出 SRAM 限制，改用 JPEG 有损压缩
    ...
    QImage quantized = ditherEpdPreview(jpegDecoded);
    QTransform rotBack;
    rotBack.rotate(-90);
